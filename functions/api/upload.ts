...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (ct.includes('multipart/form-data')) {
    ...
    if (!['pdf', 'epub'].includes(ext)) ext = inferExtFromType(f.type) || ext;
  } else {
  ...
  if (!['pdf', 'epub'].includes(ext)) return badRequest('ext must be epub or pdf');

  const bookId = crypto.randomUUID();
  const key = `user/${user.id}/books/${bookId}.${ext}`;
  const contentType = ext === 'pdf' ? 'application/pdf' : 'application/epub+zip';

  if (file) await env.R2.put(key, file.stream(), { httpMetadata: { contentType } });
  else await env.R2.put(key, request.body, { httpMetadata: { contentType } });
